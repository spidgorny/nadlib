...
class TCAConverter {
	...
	function convertTCA(array $fields) {
		...
		foreach ($fields as $field => $config) {
			if (!in_array($field, $this->skipFields)) {
				...
				$desc[$field]['label'] = $this->pi1->pi_getLL(end(explode(':', $config['label'])), $config['label']);
	...
	function convertTCA_select(array $desc, array $config) {
		if ($config['foreign_table']) {
			...
			$query = $GLOBALS['TYPO3_DB']->SELECTquery('uid, '.$name, $config['foreign_table'], '1=1 '.$this->pi1->cObj->enableFields($config['foreign_table']).' '.$config['foreign_table_where']); // may contain ONLY ORDER BY!
