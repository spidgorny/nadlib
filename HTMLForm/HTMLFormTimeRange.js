function showSliders() {
	...
	$(".time-scale").each(function () {
		...
		$slider.slider({
			...
			values: [parseInt($slider.attr("start")), parseInt($slider.attr("end"))],
			step: parseInt($slider.attr("step")),
			slide: function (event, ui) {
				var start = ui.values[0];
				start = new Date(start * 60 * 1000);
				start =
					new String(start.getUTCHours()).zf() +
					":" +
					new String(start.getUTCMinutes()).zf();
				var end = ui.values[1];
				end = new Date(end * 60 * 1000);
				end =
					new String(end.getUTCHours()).zf() +
					":" +
					new String(end.getUTCMinutes()).zf();
